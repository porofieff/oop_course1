...
/*
 * реализация класса TProcessorsWindow
 *
 *      Окно с процессорами
 *
*/
TProcessorsWindow::TProcessorsWindow(QWidget *parent)
    : QWidget(parent)
{
    setMinimumWidth(500);
    ...
    QPalette t;
    t.setColor(QPalette::Window,Qt::white);
    setAutoFillBackground(true);
    setPalette(t);

    t.setColor(QPalette::Window,Qt::gray);
    ...
    record_scroll->setMinimumHeight(750);
    record_scroll->setFixedWidth(450);
...
/*
 * реализация класса TControlWindow
 *
 *      Окно управления
 *
*/
    ...
    : QWidget(parent)
{
    ...
    t.setColor(QPalette::Window,Qt::gray);
    header = new QLabel("Управляющая панель", this);
    header->setFont(font);
    header->setGeometry(10, 10, 450, 60);
    header->setAlignment(Qt::AlignCenter);
    header->setAutoFillBackground(true);
    header->setPalette(t);

    font.setPointSize(15);
    start_btn = new QPushButton("Прогон", this);
    start_btn->setFont(font);
    start_btn->setGeometry(82, 90, 310, 40);

    reset_btn = new QPushButton("Сброс настроек", this);
    reset_btn->setFont(font);
    reset_btn->setGeometry(82, 90 + 90, 310, 40);
...
/*
 * реализация класса TTasksWindow
 *
 *      Окно со списком заданий
 *
*/
TTasksWindow::TTasksWindow(QWidget *parent)
    : QWidget(parent)
{
    ...
    setAutoFillBackground(true);
    setPalette(t);
    t.setColor(QPalette::Window, Qt::red);
    ...
    tasks_list_scroll->setMinimumHeight(300);
    tasks_list_scroll->setPalette(t);
    tasks_list_scroll->setFixedWidth(460);
    ...
    tasks_queue_scroll->setMinimumHeight(300);
    tasks_queue_scroll->setPalette(t);
    tasks_queue_scroll->setFixedWidth(460);
...
/*
 * реализация класса TSettingsWindow
 *
 *      Окно установления параметров системы
 *
*/
TSettingsWindow::TSettingsWindow(TSystemParam par, QWidget *parent)
    : QWidget(parent)
{
    setFixedWidth(420);
    ...
    header->setFont(font);
    header->setGeometry(10, 10, 400, 60);
    header->setAlignment(Qt::AlignCenter);
    header->setAutoFillBackground(true);
    header->setPalette(t);
    ...
    processors_number_l->setGeometry(10, 90, 310, 40);
    ...
    tasks_number->setGeometry(330, 90 + 50 + 90, 80, 40);

    generate_tasks_btn = new QPushButton("Сгенерировать", this);
    generate_tasks_btn->setFont(font);
    generate_tasks_btn->setGeometry(10, 90 + 50 + 90 + 50, 400, 40);
 ...
 *
*/
TMainWindow::TMainWindow(TSystem *s, QWidget *parent)
    : QWidget(parent)
{
    system = s;
    ...
    connect(sw, SIGNAL(generate(int)),
            system, SLOT(changeTasksNumber(int))); // генерация нового списка заданий
    connect(sw, SIGNAL(generate(int)),
            pw, SLOT(clear(int))); // очистка окна с отображением текущего прогона/рекорда
    connect(system, SIGNAL(show_current_tasks(QVector<TTask>)),
            tw, SLOT(show_current_tasks(QVector<TTask>))); // отображение сгенерированых заданий

    connect(sw, SIGNAL(set(int)),
            system, SLOT(changeProcessorsNumber(int))); // установка требуемого количества процессоров
    connect(sw, SIGNAL(set(int)),
            pw, SLOT(clear(int))); // очистка окна с отображением процессоров
    connect(system, SIGNAL(show_current_processors(int)),
            pw, SLOT(show_processors(int))); // отображение новых процессоров

    connect(tw, SIGNAL(to_queue(int)),
            system, SLOT(add_to_queue(int))); // добавление задачи в очередь
    connect(tw, SIGNAL(to_list(int)),
            system, SLOT(add_to_list(int))); // удаление из очереди задачи

    connect(cw, SIGNAL(start_signal(int)),
            system, SLOT(control(int))); // запуск работы процессоров
    connect(cw, SIGNAL(start_signal(int)),
            pw, SLOT(clear(int))); // очистка отображения текущего прогона
    connect(cw, SIGNAL(start_signal(int)),
            sw, SLOT(block())); // блокировка кнопок настройки параметров
    connect(cw, SIGNAL(start_signal(int)),
            pw, SLOT(clear(int))); // очистка текущего прогона

    connect(system, SIGNAL(warning_signal(int)),
            this, SLOT(warning(int))); // сигнал от ошибке
    connect(system, SIGNAL(warning_signal(int)),
            cw, SLOT(block())); // разблокировка кнопок управления
    connect(system, SIGNAL(warning_signal(int)),
            sw, SLOT(block())); // разблокировка кнопок настройки параметров

    connect(cw, SIGNAL(reset_system(int)),
            system, SLOT(reset())); // сброс параметров системы до параметров по умолчанию
    connect(cw, SIGNAL(reset_system(int)),
            pw, SLOT(clear(int))); // очистка отображения последнего прогона/рекорда
    connect(cw, SIGNAL(reset_system(int)),
            tw, SLOT(clear_tasks())); // очистка списков задач

    connect(system, SIGNAL(finished()),
            this, SLOT(completed())); // прогон завершен
    connect(system, SIGNAL(finished()),
            cw, SLOT(block())); // разблокировка кнопок управления
    connect(system, SIGNAL(finished()),
            sw, SLOT(block())); // разблокировка кнопок настройки параметров

    connect(system, SIGNAL(current_time(int)),
            pw, SLOT(show_current_time(int))); // отображение текущего времени - тактовый сигнал

    connect(system, SIGNAL(set_task_to_processor(int,TTask*)),
            pw, SLOT(set_current_task(int,TTask*))); // назначение очередной задачи на своодный процессор

    connect(system, SIGNAL(new_record(int,QVector<TTask*>)),
            pw, SLOT(show_new_record(int,QVector<TTask*>))); // информация о новом лучшем результате

    tw->show_current_tasks(system->get_tasks()); // отображение сгенерированных задач в начальном состоянии задач
    pw->show_processors(par.curProcessorsNumber); // установление заданного по умолчанию количества процессоров
...
void TMainWindow::warning(int warning_code)
{
    if (warning_code == 0)
    {
        QMessageBox::warning(this, "Предупреждение", "Не все задания добавлены в очередь!");
    }
    else if (warning_code == 1)
    {
        QMessageBox::warning(this, "Предупреждение", "Нет сгенерированных заданий!");
    }
}
